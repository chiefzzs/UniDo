...
function getExpectedInteractions(): any {
  const llmCallsPath = path.join('d:', 'learnning', '260521', 'src', 'bak', 'llm_calls.json');
...
test.describe('回放模式一致性测试 - 基于 llm_calls.json 详细检查点', () => {
  ...
  test.beforeAll(() => {
    ...
    const keepFiles = ['projects.json', 'model_configs.json', 'workspace_configs.json' , "prompts.json"];
  ...
  test('验证界面显示与 llm_calls.json 详细交互过程一致', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      // 捕获ChatManager处理记录（延迟执行，确保ChatManager已初始化）
      (window as any).setupChatManagerCapture = function() {
        if (!window.ChatManager) {
          console.log('[EventCapture] ChatManager not available yet, retrying...');
          setTimeout(() => (window as any).setupChatManagerCapture(), 100);
          return;
        }
        
        console.log('[EventCapture] Setting up ChatManager capture');
        const originalHandleWebSocketMessage = window.ChatManager.handleWebSocketMessage;
        ...
        window.ChatManager.handleWebSocketMessage = function(data: any) {
      ...
      // 启动延迟捕获设置
      setTimeout(() => (window as any).setupChatManagerCapture(), 500);
    });
    ...
    // 输出所有EventBus日志（调试用）
    console.log('\n📋 所有EventBus日志:');
    eventBusLogs.forEach((log, i) => {
      console.log(`${i+1}. ${log.args.map((a: any) => typeof a === 'string' ? a : JSON.stringify(a)).join(' ')}`);
    });
    ...
    // 检查 EventBus 订阅情况（直接检查）
    console.log('\n--- EventBus 订阅情况检查（直接检查）---');
    const eventBusSubscriptionCheck = await page.evaluate(() => {
      if (!window.EventBus) {
      ...
      // 检查普通监听器
      if (window.EventBus.listeners) {
        window.EventBus.listeners.forEach((handlers: any[], eventType: string) => {
          result.normalListeners[eventType] = handlers.length;
          result.summary.push(`${eventType}: ${handlers.length} 个普通订阅者`);
        });
      }
      
      // 检查条件监听器
      if (window.EventBus.conditionalListeners) {
        window.EventBus.conditionalListeners.forEach((handlers: any[], eventType: string) => {
          result.conditionalListeners[eventType] = handlers.length;
          result.summary.push(`${eventType}: ${handlers.length} 个条件订阅者`);
        });
      }
    ...
    if (eventBusSubscriptionCheck.error) {
      console.log(`❌ ${eventBusSubscriptionCheck.error}`);
    } else {
      console.log('📊 EventBus 订阅摘要:');
      eventBusSubscriptionCheck.summary.forEach(s => console.log(`  - ${s}`));
    }
